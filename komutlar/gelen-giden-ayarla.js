...
exports.run = async (client, message, args) => {
  ...
  if(!args[0]) return message.channel.send("Doğru kullanım `h!gelen-giden ayarla/sıfırla`")
  if(args[0] === "ayarla"){
       if (db.has(`hgbb_${message.guild.id}`)) return message.channel.send("Zaten ayarlanmış.Sıfırlamak için h!gelen-giden sıfırla")
